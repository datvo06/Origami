...
/**
 * \brief Element parser class for parsing an integer, an specialization of
 * element_parser class.
 */

template <> class element_parser<int> {
...
public:
   ...
   */
  static inline OBJ_T parse_element(char *word) { return atoi(word); }

  static const OBJ_T &convert(const int &i) { return i; }

  static bool notEq(const int &i1, const int &i2) { return !(i1 == i2); }

  static const HASH_TYPE &conv_hash_type(const OBJ_T &s) { return s; }

}; // end clas element_parser<int>
   //
template <> struct COMP_FUNC<int> {
  bool operator()(const int &lhs, const int &rhs) { return lhs == rhs; }
};
typedef struct COMP_FUNC<int> COMP_FUNC_INT;

/**
 * \brief Element parser class for parsing a string, an specialization of
 * element_parser class.
 */
template <> class element_parser<std::string> {
...
public:
  ...
  static inline OBJ_T parse_element(std::string word) { return word; }
  static OBJ_T convert(const char *s) { return parse_element(s); }
...
template <> struct COMP_FUNC<const char *> {
