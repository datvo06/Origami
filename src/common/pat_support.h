...
template <class MP> class pattern_support;

template <class MP>
std::ostream &operator<<(std::ostream &, const pattern_support<MP> &);

/**
 * \brief Generic Pattern Support Class that takes care of support counting of a
 * pattern.
 *
 * Templated with Mining property, since support counting depends on different
 * mining properties.
 */
template <class MP> class pattern_support {
...
public:
  // constructor
  inline pattern_support(const int &s = 0, bool v = 0) : _sup(s), _valid(v) {}

  inline bool is_valid(const int &minsup) const { return _sup >= minsup; }

  inline bool is_freq(const int &ms) const { return _sup >= ms; }

  pattern_support<MP> &operator=(const pattern_support<MP> &rhs) {
    // pattern_support<MP> lhs(rhs._sup, rhs._valid);
    if (this == &rhs)
      return *this;
    _sup = rhs._sup;
    _valid = rhs._valid;
...
std::ostream &operator<<(
    std::ostream &ostr,
    const pattern_support<
        proplist<Fk_Fk, proplist<vert_mine, proplist<induced, T>>>> &ps) {
...
/**
 * \brief Pattern Support class specialized for Fk X Fk join type and induced
 * mining.
 *
 */
template <class T>
class pattern_support<
    proplist<Fk_Fk, proplist<vert_mine, proplist<induced, T>>>> {

public:
  // constructor
  pattern_support(const int &is = 0, const int &s = 0, bool v = 0)
      : _isup(is), _esup(s) {}
  ...
  bool is_freq(const int &minsup) const { return _isup >= minsup; }
  ...
  void set_vals(const pattern_support<
                proplist<Fk_Fk, proplist<vert_mine, proplist<induced, T>>>>
                    *const &s) {
    _esup = s->_esup;
    _isup = s->_isup;
